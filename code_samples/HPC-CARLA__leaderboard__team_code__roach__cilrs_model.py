...
class RoachILPolicy(nn.Module):
    """Inference-only adapter around the verbatim CoILICRA network so it can be driven
    by the repo's generic ``TorchModelRunner`` (which calls the model with a single
    dict argument and loads ``checkpoint[state_dict_key]`` via ``load_state_dict``).

    This is NOT part of the original Roach code — it only bridges calling conventions:
      * ``forward(input_dict)`` unpacks ``im``/``state`` and adds the singleton temporal
        axis so the tensor matches CoILICRA.forward's expected ``(b, c, t, h, w)``
        (cilrs_agent stacks a single frame with ``th.stack(..., dim=1)`` +
        ``forward_branch``'s ``im.unsqueeze(0)``; here the batch axis already exists).
      * ``load_state_dict`` re-prefixes the Roach checkpoint's ``policy_state_dict``
        (keys like ``perception.*`` / ``mu_branches.*``) onto our ``net.*`` submodule.

    Branch selection + Beta->action decoding are intentionally left OUT of the model
    and live in team_code.roach.modules (glue), so the vendored network stays verbatim.
    """

    def __init__(self, **coil_kwargs):
        super(RoachILPolicy, self).__init__()
        self.net = CoILICRA(**coil_kwargs)

    def forward(self, input_dict):
        im = input_dict['im']        # (B, C, H, W), ImageNet-normalized float
        state = input_dict['state']  # (B, S) float
        if im.dim() == 4:
            # (B, C, H, W) -> (B, C, T=1, H, W) == CoILICRA's (b, c, t, h, w)
            im = im.unsqueeze(2)
        return self.net(im, state)

    def load_state_dict(self, state_dict, strict=True):
        if not any(str(k).startswith('net.') for k in state_dict.keys()):
            state_dict = OrderedDict(
                ('net.' + str(k), v) for k, v in state_dict.items()
            )
        return super(RoachILPolicy, self).load_state_dict(state_dict, strict=strict)