...
class RainNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=RAIN,
        ...
        self.norm_type_list = [get_norm_layer('instance'), get_norm_layer('instance')]#get_norm_layer('rain')]
        self.norm_type_indicator = norm_type_indicator
        # -------------------------------Network Settings-------------------------------------
        ...
        self.batchnorms_first_half = nn.ModuleList([
            self.norm_type_list[self.norm_type_indicator[0]](32),
            self.norm_type_list[self.norm_type_indicator[1]](64),
            self.norm_type_list[self.norm_type_indicator[2]](128),
            self.norm_type_list[self.norm_type_indicator[3]](128),
            self.norm_type_list[self.norm_type_indicator[4]](128),
            self.norm_type_list[self.norm_type_indicator[5]](128),
            self.norm_type_list[self.norm_type_indicator[6]](128),
        ])
        ...
        self.batchnorms_second_half = nn.ModuleList([

            self.norm_type_list[self.norm_type_indicator[7]](128),
            self.norm_type_list[self.norm_type_indicator[8]](64),
            self.norm_type_list[self.norm_type_indicator[9]](32),
        ])
    ...
    def forward(self, x, mask):
        temp = []
        #x = self.first_conv(x)
        for i, conv in enumerate(self.first_half):
            x = self.lrelu(x)
            x = conv(x)
            temp.append(x)
            #print(i)
            if i < 6:
                x = self.batchnorms_first_half[i](x)

        for i, conv in enumerate(self.second_half):
            x = self.relu(x)
            x = conv(x)
            x = self.batchnorms_second_half[i](x)
            #print(temp[2-i].shape, x.shape)
            x = torch.cat([temp[2-i], x], 1)
